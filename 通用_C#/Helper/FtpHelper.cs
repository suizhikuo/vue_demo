...
namespace Common
{
    ...
    {
        public delegate void chpro(double sum, double size);
        ...
        //服务器上文件总大小   
        public long serverfilesize = 0;
        //已经下载了多少   
        public long filesize = 0;
        public chpro delechpro;
        ...
        public bool Download(string serverName, string localName)
        {
            ...
            using (FileStream fs = new FileStream(localName, FileMode.OpenOrCreate)) //创建或打开本地文件   
            {
                ...
                using (FtpWebResponse response = (FtpWebResponse)request.GetResponse())
                {
                    ...
                    byte[] buffer = new byte[1024 * 1024];//4K   
                    serverfilesize = GetFileSize(serverName);
                    ...
                    while (count > 0)
                    {
                        ...
                        filesize += count;
                        if (delechpro != null)
                            delechpro(Convert.ToDouble(serverfilesize), Convert.ToDouble(filesize));
