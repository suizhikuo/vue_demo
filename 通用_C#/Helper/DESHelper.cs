...
namespace Common
{
    ...
    public class DESHelper
    {
        ...
        /// <returns></returns>
        public static string Encrypt(string beinetstr, string beinetkey)
        {
        ...
        /// <param name="beinetkey">密钥</param>
        /// <returns></returns>
        public static string Decrypt(string beinetstr, string beinetkey)
        {
            if (string.IsNullOrEmpty(beinetkey)) return null;
        ...
        static string StringToHex(string s)
        {
            ...
            {
                                sb.Append(hexes[RM(s[i], 4)]).Append(hexes[s[i] & 0xf]);
        ...
        static string HexToString(string s)
        {
                        StringBuilder sb = new StringBuilder();
        ...
        /// <returns></returns>
        static string DES(string beinetkey, string message, bool encrypt, bool mode, string iv)
        {
            ...
            int[] keys = DESCreateKeys(beinetkey);
            int m = 0;
            int i, j;
            ...
            string tempresult = string.Empty;
                        ...
                        StringBuilder sbTempResult = new StringBuilder();
            ...
            while (m < len)
            {
                ...
                if (encrypt)
                {
                    //arrInt = new int[]{ RM(left, 24), (RM(left, 16) & 0xff), (RM(left, 8) & 0xff), (left & 0xff), RM(right, 24), (RM(right, 16) & 0xff), (RM(right, 8) & 0xff), (right & 0xff) };
                                        sbTmp.Clear();
                                        sbTmp.Append((char)RM(left, 24)).Append((char)(RM(left, 16) & 0xff)).Append((char)(RM(left, 8) & 0xff)).Append((char)(left & 0xff)).Append((char)RM(right, 24)).Append((char)(RM(right, 16) & 0xff)).Append((char)(RM(right, 8) & 0xff)).Append((char)(right & 0xff));
                                        sbTempResult.Append(sbTmp.ToString());
                ...
                else
                {
                    ...
                    if (tmpch != 0)
                                        {
                                                sbTempResult.Append((char)tmpch);
                ...
                if (chunk == 512)
                {
                    //result.AddRange(tempresult);tempresult.Clear(); 
                                        ...
                                        sbResult.Append(sbTempResult.ToString());
                                        sbTempResult.Clear();
            ...
            //return result.ToArray();
                        ...
                        sbResult.Append(sbTempResult.ToString());
                        return sbResult.ToString(); //result + tempresult;
        }


        static int[] DESCreateKeys(string beinetkey)
        {
            //declaring this locally speeds things up a bit
